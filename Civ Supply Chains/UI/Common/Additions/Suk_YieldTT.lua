...
local function CSC_IsPositiveProperty(owner, propertyName)
	...
	local value = tonumber(owner:GetProperty(propertyName) or 0);
...
local function CSC_FormatSignedYield(value)
...
local function CSC_ReclassifyOutgoingFood(tEntries, amount)
	...
	local outgoingLabel = CSC_GetTooltipLabel(CSC_LOC_OUTGOING_ROUTES);
	if modifierLabel == nil or outgoingLabel == nil then return; end

	local modifierIndex, modifierValue = nil, nil;
	local outgoingIndex, outgoingValue = nil, nil;
	for index, entry in ipairs(tEntries) do
		if entry[2] == "" and entry[3] == modifierLabel and modifierIndex == nil then
			modifierIndex = index;
			modifierValue = tonumber(entry[1]);
		elseif entry[2] == "" and entry[3] == outgoingLabel then
			outgoingIndex = index;
			outgoingValue = tonumber(entry[1]);
		end
	end
	...
	if outgoingIndex ~= nil and outgoingValue ~= nil then
		tEntries[outgoingIndex][1] = CSC_FormatSignedYield(outgoingValue + amount);
	end

	local adjustedModifierValue = modifierValue - amount;
	local modifierWasRemoved = math.abs(adjustedModifierValue) < 0.001;
	if modifierWasRemoved then
		table.remove(tEntries, modifierIndex);
	else
		tEntries[modifierIndex][1] = CSC_FormatSignedYield(adjustedModifierValue);
	end
