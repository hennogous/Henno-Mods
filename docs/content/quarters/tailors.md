---
title: Tailors' Quarter
---

# 2. Tailors' Quarter

![Tailors' Quarter](../images/Civ%20Modding%20Projects%20-%20Tailors'%20Quarter.jpg)

| | Standard game | Mods |
|---|---|---|
| {{base}} Base Materials | Cotton, *Sheep* | Bamboo (R2), Flax (CSC), Hemp (CH), *Llamas* (LAR) |
| {{spec}} Specialty Materials | Dyes, Silk, Silver | Cashmere (R2), Gold (R2 / SR) |

* Main yield focus: {{culture}} Culture
* Unlocks at Craftsmanship
* Base cost: 60 {{production}} Production
* Maintenance cost: 1 {{gold}} Gold

---

* +1 {{production}} Production from every 2 adjacent river edges.
* +1 {{production}} Production from each adjacent {{base}} Base or {{spec}} Specialty Materials resource from this supply chain.
* +1 {{gold}} Gold from each adjacent {{sales}} Harbor, and +1 {{production}} Production in return.
* +1 {{gold}} Gold from each adjacent {{sales}} Commercial Hub, Holy Site and Theater Square, and +1 {{culture}} Culture in return.
* with City Lights:
   * +1 {{production}} Production from each adjacent {{goods}} Rural Community, and +1 {{culture}} Culture in return.
   * +1 {{gold}} Gold from each adjacent {{sales}} Urban Borough, and +1 {{culture}} Culture in return.

## 1. Materials

* {{base}} Base Materials improvements: +1 {{production}} Production and +1 {{gold}} Gold from each adjacent Textile Workshop.
   * +2 {{production}} Production and +2 {{gold}} Gold to an Industry, and +3 {{production}} Production and +3 {{gold}} Gold to a Corporation.
* {{spec}} Specialty Materials improvements: +1 {{production}} Production and +1 {{gold}} Gold from each adjacent Fashion House.
   * +2 {{production}} Production and +2 {{gold}} Gold to an Industry, and +3 {{production}} Production and +3 {{gold}} Gold to a Corporation.

## 2. Textile Workshop

* Unlocks at Construction
* Costs 80 {{production}} Production
* Maintenance: -2 {{gold}} Gold

* +1 {{culture}} Culture from each adjacent {{base}} Base Materials improvement.
   * +2 {{culture}} Culture from an Industry, and +3 {{culture}} Culture from a Corporation.
* +1 {{production}} Production and +1 {{gold}} Gold from the local Tailor and Fashion House, providing +1 {{culture}} Culture to each in return.
* +1 {{production}} Production and +1 {{gold}} Gold from each adjacent Lighthouse, and +1 {{production}} Production in return.
* At Naval Tradition, a Textile Workshop adjacent to an improved {{base}} Base Materials resource unlocks:
   * A Dockmaster is established in an adjacent Harbor if it has a Lighthouse.
   * The Dockmaster provides the city with +20% {{production}} Production toward Renaissance Era or earlier naval units, +1 {{citizen}} Citizen slot and +1 {{admiral}} Great Admiral point per turn.

## 3. Tailor

* Unlocks at Guilds
* Requires a Textile Workshop in the Quarter
* Costs 160 {{production}} Production
* Maintenance: -2 {{gold}} Gold

* +1 {{production}} Production and +1 {{gold}} Gold to the local Textile Workshop, in return for +1 {{culture}} Culture.
* +1 {{citizen}} Citizen slot, and +2 {{culture}} Culture and +1 {{gold}} Gold to {{citizen}} Citizens in the Quarter.
* +1 {{amenity}} Amenity in the city.
* +0.1 {{culture}} Culture per {{citizen}} Citizen to the city of each adjacent Temple or Market, and +0.1 {{production}} Production and +0.1 {{gold}} Gold per {{citizen}} Citizen to the Tailor's city in return.
* At Divine Right, a Tailor adjacent to an improved {{base}} Base Materials resource unlocks:
   * A Sacristan is established in an adjacent Holy Site if it has a Temple.
   * The Sacristan provides the city with +10% {{faith}} Faith, +1 {{citizen}} Citizen slot and +1 {{prophet}} Great Prophet point per turn.
* +1 {{culture}} Culture bonus to trade routes to the city, and +1 {{production}} Production and +1 {{gold}} Gold to the Quarter in return, if the origin city does not have a Tailors' Quarter. +1 {{amenity}} Amenity to the origin city.

## 4. Fashion House

* Unlocks at Humanism
* Requires a Textile Workshop in the Quarter
* Costs 250 {{production}} Production
* Maintenance: -3 {{gold}} Gold

* +1 {{production}} Production and +1 {{gold}} Gold to the local Textile Workshop, in return for +1 {{culture}} Culture.
* +1 {{culture}} Culture from each adjacent {{spec}} Specialty Materials improvement.
   * +2 {{culture}} Culture from an Industry, and +3 {{culture}} Culture from a Corporation.
* +1 {{citizen}} Citizen slot, and +1 {{culture}} Culture and +2 {{gold}} Gold to {{citizen}} Citizens in the Quarter.
* +1 {{amenity}} Amenity in all cities within 6 tiles.
* +1 {{culture}} Culture for every 5 {{citizen}} Citizens in the city to each adjacent Amphitheater, Bolshoi Theatre, Broadway or Sydney Opera House, and +1 {{production}} Production and +1 {{gold}} Gold to the Fashion House in return.
* At Opera and Ballet, a Fashion House adjacent to improved {{base}} Base and {{spec}} Specialty Materials resources unlocks:
   * A Stage Manager is established in an adjacent Theater Square with an Amphitheater, or in the Tailors' Quarter if it is adjacent to Bolshoi Theatre, Broadway or Sydney Opera House.
   * The Stage Manager provides the city with +10% {{culture}} Culture, +1 {{citizen}} Citizen slot and +1 {{artist}} Great Artist point per turn.
* +1 {{culture}} Culture bonus to trade routes to the city, and +1 {{production}} Production and +1 {{gold}} Gold to the Quarter in return, if the origin city does not have a Tailors' Quarter. +1 {{amenity}} Amenity to the origin city.
