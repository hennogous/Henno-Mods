...
NAME='CSC_BLACKSMITH_Workshop_v1'
...
plaster=mat('Preview | warm lime plaster',(.63,.49,.31))
wood=mat('Preview | oak structure',(.19,.105,.052))
...
brick_shader(roof,(.48,.145,.055),(.62,.245,.085),(.27,.10,.04),1,.46,.37)
brick_shader(stone,(.32,.31,.26),(.44,.41,.33),(.235,.223,.187),1,.52,.30)
...
def roof_sheet(name,line,y0,y1):
    vs=[(x,y,z) for y in (y0,y1) for x,z in line];n=len(line)
...
beam('06 | ridge cap',(-.88,-1.73,4.81),(-.88,1.72,4.81),.22)
for y in [-1.6,1.59]:
    box('07 | ridge terminal',(-.88,y,4.83),(.36,.30,.37),wood,taper=.82)
...
box('13 | door opening',(-1.48,-1.365,.91),(.64,.045,1.46),dark)
box('14 | oak door',(-1.48,-1.40,.9),(.56,.065,1.34),wood)
for z in [.43,1.27]:box('15 | door strap',(-1.48,-1.444,z),(.48,.02,.055),iron)
...
box('20 | loft dark opening',(-.88,-1.365,3.18),(.45,.04,.61),dark)
...
roof_sheet('30 | sloping forge canopy',[(.29,2.61),(1.15,2.41),(2.25,2.24)],-1.22,1.38)
...
box('40 | forge chimney shaft',(1.13,.87,2.58),(1.12,.99,5.16),stone,top=False,taper=.66)
...
bpy.ops.object.light_add(type='POINT',location=(cx,-.15,.9));li=bpy.context.object;studio_obj(li);li.name='Forge glow';li.data.energy=18;li.data.color=(1,.23,.035);li.data.shadow_soft_size=.3
...
scene.render.image_settings.file_format='PNG';scene.render.filepath=os.path.join(OUT,'blacksmith-render-v1.png')
bpy.ops.object.select_all(action='DESELECT')
