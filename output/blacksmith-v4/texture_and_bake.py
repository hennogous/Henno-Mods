...
OUT=ROOT+'/output/blacksmith-v4';TEX=OUT+'/Textures';NAME='CSC_BLACKSMITH_Workshop_v4'
source=bpy.data.scenes['CSC_BLACKSMITH_Workshop_v4_source']
...
for source_ob in source_collection.objects:
    ...
    for poly in me.polygons:
        ...
        if ob.name[:2] in ['60','70']:pr='endgrain' if abs(n.z)>.88 else 'wood'
        if ob.name.startswith('07'):pr='wood'
        if ob.name[:2] in ['04','05']:
            stations=[i%14 if i%14<7 else 13-i%14 for i in poly.vertices]
            pr='endgrain' if min(stations)==max(stations) else 'wood'
        ...
        for li,p in zip(poly.loop_indices,coords):
            if ob.name[:2] in ['04','05'] and pr=='wood':
                # Follow the swept board instead of projecting straight grain
                # across its curve. Source vertices are outer profile + inner
                # reversed profile, duplicated along the board's depth.
                vi=me.loops[li].vertex_index % 14
                station=vi if vi<7 else 13-vi
                u=(.08 if vi<7 else .28) if abs(n.y)>.8 else .08+(p.y-bounds[1][0])*.55
                v=.035+.90*arc[station]/arc[-1]
            elif region=='roof' and ob.name.startswith('03'):
                u=(p.y+1.62)/3.20
                d=roof_distance(p.x)
                v=(arc[3]-d)/arc[3] if poly.center.x<roofline[3][0] else (d-arc[3])/(arc[-1]-arc[3])
            elif region=='roof':
...
sc.render.image_settings.file_format='PNG';sc.render.filepath=OUT+'/blacksmith-render-v4.png'
