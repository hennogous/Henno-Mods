...
NAME='CSC_BLACKSMITH_Workshop_v4_source'
...
# Bargeboards have a true perpendicular width, not a vertical-only offset.
def trim(name,line,y0,y1,thick):
    # Mitered offset polyline: maintain visible timber width on steep slopes.
    normals=[]
    for (xa,za),(xb,zb) in zip(line,line[1:]):
        tangent=Vector((xb-xa,zb-za)).normalized()
        normals.append(Vector((tangent.y,-tangent.x)))
    outer=[];inner=[]
    for i,point in enumerate(line):
        a=normals[max(0,i-1)];b=normals[min(i,len(normals)-1)]
        bisector=(a+b).normalized()
        width=thick*(1.0+.10*math.sin(i*.8))
        offset=bisector*(width/max(.25,bisector.dot(a)))
        p=Vector(point)
        outer.append(tuple(p-offset*.20));inner.append(tuple(p+offset*.80))
    return profile_prism(name,outer+list(reversed(inner)),y0,y1,wood)
trim('04 | front sweeping bargeboard',roofline,-1.84,-1.50,.29)
trim('05 | rear sweeping bargeboard',roofline,1.48,1.80,.27)
vs=[]
for y,z in [(-1.73,4.92),(0,4.72),(1.72,4.92)]:
    vs.extend([(-1.04,y,z-.17),(-.72,y,z-.17),(-.72,y,z+.17),(-1.04,y,z+.17)])
...
for y in [-1.6,1.59]:
    box('07 | ridge terminal',(-.88,y,4.95),(.46,.43,.48),wood,taper=.78)

# Framing, door, and loft opening. Texture will provide joinery and fine lines.
for x in [-1.96,.29]:
    beam('10 | front corner post',(x,-1.34,.28),(x,-1.34,2.7),.30)
beam('11 | front cross beam',(-2.03,-1.36,2.66),(.40,-1.36,2.66),.30)
beam('12 | front floor beam',(-1.98,-1.36,1.92),(.30,-1.36,1.92),.23)
...
beam('31 | front canopy fascia',(.29,-1.24,2.56),(2.30,-1.24,2.12),.28)
beam('32 | side canopy fascia',(2.25,-1.31,2.10),(2.25,1.45,2.10),.28)
for y in [-1.13,1.19]:
    box('33 | post stone foot',(2.03,y,.20),(.48,.48,.44),stone,taper=.80)
    beam('34 | canopy upright',(2.03,y,.32),(2.03,y,2.15),.32)
    beam('35 | diagonal knee brace',(2.03,y,1.65),(1.49,y,2.31),.19)
...
for ob in parts:
    ...
    if ob.name.startswith('34 |'):
        for v in ob.data.vertices:v.co.x+=.13*(1-v.co.z/2.3)
...
# Broader body, slightly lower center of gravity, and heavier tapering posts.
for ob in parts:
    if ob.name.startswith('10 |'):
        cx=sum(v.co.x for v in ob.data.vertices)/len(ob.data.vertices)
        for v in ob.data.vertices:
            factor=1.20-.27*min(1,max(0,v.co.z/2.7))
            v.co.x=cx+(v.co.x-cx)*factor
            v.co.y=-1.34+(v.co.y+1.34)*factor
    for v in ob.data.vertices:
        if v.co.x<.35:v.co.x=.35+(v.co.x-.35)*1.12
        v.co.z*=.95
    ob.data.update()
roofline=[(.35+(x-.35)*1.12 if x<.35 else x,z*.95) for x,z in roofline]
...
scene.render.image_settings.file_format='PNG';scene.render.filepath=os.path.join(OUT,'blacksmith-source-v4.png')
bpy.ops.object.select_all(action='DESELECT')
