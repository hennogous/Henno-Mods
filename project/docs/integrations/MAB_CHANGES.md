...
> **Documentation audit — 2026-09-12: Historical.** June 23 branch/PR/commit record. Remote PR status and the currently installed Windows branch were not rechecked. The final local-only claim conflicts with its own table listing an integration PR; do not treat either as live status.
> Classification: Dependency history. See the [full audit](../DOCUMENT-AUDIT.md).
