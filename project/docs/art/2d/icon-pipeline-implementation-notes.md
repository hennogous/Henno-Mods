...
`project/tools/comfyui/icon_pipeline/icon_img2img.py` writes final outputs beside the input
image. Its generated Canny guide and raw pre-postprocess image are temporary
files and are cleaned up after the run.

`project/tools/comfyui/icon_pipeline/icon_postprocess.py` defaults to:
