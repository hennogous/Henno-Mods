...
> **Documentation audit — 2026-09-12: Historical.** Learning record includes unique-building direction predating the agreed reusable-kit plan. Technical experiments remain useful; production direction comes from the new plan. Linked output/blacksmith-v5/README.md is unavailable at its recorded location.
> Classification: Workflow development record. See the [full audit](../../DOCUMENT-AUDIT.md).

Latest calibration: [direct Firaxis guidance review](firaxis-guidance-review.md), including corrections to the earlier AO-binding inferences.
...
The initial skill was developed in this repository. It is now maintained in the
standalone [civ6-art repository](https://github.com/hennogous/civ6-art),
installed alongside the CSC and Civ VI modding skills. Its modeling rules remain
separate from CSC asset paths, trial history and art contracts in project docs/specs.
`civ6-art` owns art creation and integration; `civ6-gameplay` owns gameplay rules and logic.

## Scale reference

Henno designated `Working Files/3D Art/Level 2/CSC_Level_2.blend` as the correct scale
for a two-storey building with a raised roof (2026-09-11).
The model includes a semi-arbitrary amount of underground foundation. Henno clarified
that the actual architectural scale runs from the bottom of the front door upward.
Use that threshold as ground level and exclude the buried foundation when comparing
storey and roof heights. The full mesh dimensions below include the foundation;
125.3685 is therefore not the above-ground building height.

Measured directly from `CSC_Level_2_Bldg` in Blender 5.1.2 after inspecting the door
geometry: **109.4165 Blender units from the front-door bottom to the highest roof
point**. Use **109.42 units** as the practical above-ground height benchmark.
The door panel's bottom edge is vertices 342 and 344 (zero-based indices, face 382),
at world Z = -0.3746624 and -0.3695676. Its midpoint gives the ground datum
Z = -0.3721150; the highest roof vertex is Z = 109.0444260. Height is the difference.
The slight threshold slope changes the result by only ±0.00255 units. The lowest
foundation vertex is Z = -16.3240414, about 15.9519 units below this datum.
These coordinates describe the inspected source mesh, with no file modification.
Mac source: `/Users/henno.gous/Library/CloudStorage/GoogleDrive-henno.gous@gmail.com/Other computers/My PC/Working Files/3D Art/Level 2/CSC_Level_2.blend`.
Windows source: `C:\Users\Shadow\Desktop\Working Files\3D Art\Level 2\CSC_Level_2.blend`.

Read-only inspection in Blender 5.1.2 found mesh `CSC_Level_2_Bldg` with dimensions
107.9516 × 108.8722 × 125.3685 Blender units (X/Y/Z), location zero and scale 1/1/1.
The scene uses Metric units with unit scale 0.01. Armature `CSC_Level_2` has location
Z = -3.610043 and scale 1/1/1. These are source-file measurements, not a newly verified
conversion or in-game placement. Compare imported references in the same coordinate
system; do not reinterpret the displayed metric units as a new runtime scale.

Henno clarified during the textile-workshop design (2026-09-11) that this reference
includes a semi-arbitrary amount of underground foundation. Its full bounding-box
height is therefore not the above-ground building height. Preserve the authored
vertical placement and compare ground-relative walls, eaves and roof heights;
do not align the lowest mesh vertices to the ground or scale another building
from the foundation-inclusive height. Verify the intended ground datum separately.
Use door openings as a human-scale cross-check before accepting overall massing:
Henno identified the first textile-workshop blockout as oversized from its doors.
...
[prompt templates and material directives](https://github.com/hennogous/civ6-art/blob/main/references/texture-image-generation.md),
...
The skill's [performance reference](https://github.com/hennogous/civ6-art/blob/main/references/performance.md)
