...
FINAL_TARGET_SATURATION = float(os.environ.get("CSC_ICON_FINAL_TARGET_SATURATION", "0"))
# Optional final subject grade. Use the SV Blender frontend's alpha-masked
# saturation metric and capped ImageEnhance.Color factor; 0 disables the pass.
FINAL_MAX_SATURATION_FACTOR = float(os.environ.get("CSC_ICON_FINAL_MAX_SATURATION_FACTOR", "1.85"))
...
def process_icon(input_path, output_path, canny_path=None):
    ...
    if FINAL_TARGET_SATURATION > 0:
        ...
        if subject.any():
            ...
            def measured_saturation(factor):
                rgb = np.asarray(enhancer.enhance(factor), dtype=np.float32)[subject] / 255.0
                saturation = (rgb.max(axis=1) - rgb.min(axis=1)) / np.maximum(rgb.max(axis=1), 1e-6)
                return float(saturation.mean())

            lower, upper = 1.0, max(1.0, FINAL_MAX_SATURATION_FACTOR)
            for _ in range(16):
                middle = (lower + upper) / 2
                if measured_saturation(middle) < FINAL_TARGET_SATURATION:
                    lower = middle
                else:
                    upper = middle
            factor = (lower + upper) / 2
            alpha = img.getchannel("A")
            img = enhancer.enhance(factor).convert("RGBA")
            img.putalpha(alpha)
            print(f"  4. SV-style final saturation: target={FINAL_TARGET_SATURATION:.3f}, achieved={measured_saturation(factor):.3f}, factor={factor:.3f}")
